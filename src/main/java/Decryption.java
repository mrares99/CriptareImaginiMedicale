...
//    }
    ...
    public BufferedImage doDecryption(long seed, int [][] matrix, BufferedImage colorChannel, String channel) throws NoSuchAlgorithmException {
        int width=colorChannel.getWidth(),height= colorChannel.getHeight();
        SecureRandom random = SecureRandom.getInstance("SHA1PRNG");
        random.setSeed(seed);
        ...
        BufferedImage outputBufferedImage=recreateImageFromList(bitsFromRows,width,height,channel);
        return outputBufferedImage;
    }
...
//    }
    ...
    public List<Integer> doCircularLeftShift(List<Integer> inputList,int distance){
        Collections.rotate(inputList,(-1)*distance);
        return inputList;
    }
